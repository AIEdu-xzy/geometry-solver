...
class SupplementaryAngleSum1(Theorem):

    name = "supplementary angle sum 1"
    ...
    def index(self, indexer: Indexer):
        ...
        for cond in conds:
            ...
            if angle1_cond.attr_value is not None \
                    and angle2_cond.attr_value is None:
                ret.append(([cond, angle1_cond], angle2_cond))
            elif angle1_cond.attr_value is None \
                    and angle2_cond.attr_value is not None:
                ret.append(([cond, angle2_cond], angle1_cond))

        return ret

    def deduct(self, sources: List[Condition], target: Condition):
        target.attr_value = 180 - sources[1].attr_value
